...
**Техдолг (осознанный):**
- Покрытие тестами: `train/data.py` покрыт (tests/test_data.py), 
  `train/lora_train.py` проверяется через production-метрики 
  (eval loss, публикация HF, cron-запуски). 
  Юнит-тесты на обучение не добавлены: требуют mock HF Hub, 
  ROI низкий для single-dev проекта.
